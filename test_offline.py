...
def test_config_and_logging():
    ...
    cfg = Config(resource="USB::TEST", scl="CH1", sda="CH2", vdd=1.8,
                 probe_delay_s=0, high_pct=70, low_pct=30,
                 limits={"tsu_dat": {"min_ns": 100}, "tscl_fall": {"max_ns": 300}})
    assert cfg.evaluate("tsu_dat", 388e-9).status == "PASS"
    assert cfg.evaluate("tsu_dat", 50e-9).status == "FAIL"
    assert cfg.evaluate("tscl_fall", 400e-9).status == "FAIL"
    assert cfg.evaluate("thd_dat", 10e-9).status == "NO LIMIT"
